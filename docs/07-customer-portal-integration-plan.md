...
**Update:** a working customer portal already exists inside `Premier-CRM` at `apps/web/app/portal/` — real Supabase Auth signup/signin, a `customer_accounts` link table, match-or-create linking logic on signup, and a dashboard querying real `service_requests`/`customer_properties` data. This is confirmed customer-facing infrastructure (not staff tooling — it explicitly excludes itself from `org_members`), separate from `/login`, the actual staff CRM login. Decision: **port this to the website's existing design rather than rebuild it from scratch, and reuse its schema/auth logic as the reference implementation.** The CRM's own `/portal` route is retired once the website version is confirmed working — not before, so there's a fallback while the website integration is in progress.
...
- Supabase Auth handles portal sign-in. Each portal user is a Supabase Auth user linked to exactly one `customers.id` — **already implemented** via the `customer_accounts` table in `Premier-CRM` (`auth_user_id`, `customer_id`, `org_id`, `email`, `status`, `invited_at`, `accepted_at`, upserted on `auth_user_id` conflict). Reuse this table and its shape rather than designing a new one.
...
2. ~~Auth wiring~~ — **Already built in `Premier-CRM`** (`customer_accounts` table, signup/signin server actions, match-or-create linking). No new migration needed for this piece — port the existing logic into the website repo instead of designing it from scratch. Do verify RLS per the Auth model section above before relying on it.
...
8. **Retire the CRM's `/portal`** — once the website version is confirmed working end-to-end (signup, signin, dashboard, request submission all verified), remove `apps/web/app/portal/` from `Premier-CRM`. Not before — it stays as a working fallback while the website integration is in progress.
