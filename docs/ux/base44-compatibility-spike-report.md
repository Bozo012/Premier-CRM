...
- **The relocated business-rule defect has been corrected — in the final Today implementation, not on this branch.** `_lib/view-model.ts`'s `buildQuoteActivityRows()` contained a genuine workflow-relevance rule (which quote activity counts as "still actionable"), not mere presentation normalization. The corrected version of this rule now lives in `packages/db/queries/today-actions.ts`'s `getTodayQuoteActivity()`, alongside `getTodayActionItems()`, on `feature/forge-v1.1-today-redesign` (PR2 of the Forge V1.1 UX modernization program — see `docs/ux/forge-v1.1-today-redesign.md`). **This spike branch itself remains unmerged and untouched** — the fix was made fresh in the real implementation, not by patching this branch. If this branch is ever considered for archival/deletion, that decision is separate and still pending.
- **The final Today implementation (`feature/forge-v1.1-today-redesign`), not this spike branch, is the authoritative production candidate** for `/today`. This spike remains reference-only: architectural proof that the 3-layer pattern is compatible with Forge's existing boundary, superseded in every functional respect by the real implementation.
