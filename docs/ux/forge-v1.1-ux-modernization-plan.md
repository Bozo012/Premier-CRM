...
**Base44 Today visual integration**: implemented on `feature/forge-v1.1-today-base44-visual-integration` (branched from `main` @ `cc795b5`), PR opened, **not merged**. Adopts the approved Base44 Today visual reference (`Bozo012/Forge-Base44-UX` @ `adee72e`) onto Today's existing Layer 3 `BASE44-REPLACEABLE` seams, plus a new Forge-owned Light/Dark/System appearance setting and a re-themed shared nav shell. Full record: `docs/ux/base44-today-sync-and-portability-audit.md` §13, `docs/ux/forge-v1.1-today-redesign.md`. **Estimates (PR3) and Site Inspection (PR4) remain not started** — they are next, in that order, once the Today visual integration is reviewed and merged. Photo (§9.1) and dictation (§9.2) functional work remain entirely pending — untouched by both the Today redesign and this visual integration, exactly as scoped.
