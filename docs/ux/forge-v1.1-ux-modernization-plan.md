...
**Base44 Today visual integration — merged.** PR #110 merged to `main` at commit `daa917b`, automatically deployed to production. Adopted the approved Base44 Today visual reference (`Bozo012/Forge-Base44-UX` @ `adee72e`) onto Today's existing Layer 3 `BASE44-REPLACEABLE` seams, plus a new Forge-owned Light/Dark/System appearance setting and a re-themed shared nav shell. Full record: `docs/ux/base44-today-sync-and-portability-audit.md` §13, `docs/ux/forge-v1.1-today-redesign.md`. **Estimates (PR3) is next**, then Site Inspection (PR4) — both **not yet started**, pending Kevin's separate authorization to begin. Photo (§9.1) and dictation (§9.2) functional work remain entirely pending — untouched by both the Today redesign and this visual integration, exactly as scoped. No `forge-v1.1` tag exists yet.
