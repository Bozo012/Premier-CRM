...
## Double-billing: closed at the database level (follow-up to the app-layer-only gap above)

Independent verification correctly flagged the app-layer-only mitigation above as insufficient for final authority: the pre-insert existence check in `addExpenseChargeToInvoice` is a check-then-act race — two concurrent requests can both pass it before either insert commits. This was resolved with a real DB constraint before this PR was marked ready for review.

**Uniqueness-invariant audit (before writing any migration).** Confirmed via full-repository search plus a live data audit, not assumed from current UI behavior:
- `invoice_line_items.source_expense_id` is `UUID REFERENCES public.expenses(id) ON DELETE SET NULL`, nullable, no existing uniqueness (migration `20260805075928_forge_expenses_foundation.sql`).
- Exactly one write path exists anywhere in the repository (`addExpenseChargeToInvoice`) — grepped the whole app layer, confirmed no other code ever sets this column.
- `expenses.status` has a `'partially_invoiced'` enum literal, but `deriveApprovedExpenseStatus()` (the only function that transitions expense status) never produces it — it's an unbuilt, unwritten future concept, not something live code relies on splitting one expense across multiple line items.
- `voidInvoice()` never touches a linked expense's `invoice_id`/`status` — historical linkage is permanent; a voided invoice does not free its expense for rebilling. No credit/reversal mechanism exists anywhere that reuses `source_expense_id`.
- Verdict: the intended business rule — "one expense represents one incurred cost and may be billed to the customer at most once" — is fully confirmed by repository evidence. Database uniqueness is correct and safe.

**Existing-data audit (read-only, before applying anything).** Queried both `premier-crm-e2e` and `premier-crm-prod` (read-only, counts/IDs only, no customer data exposed): zero non-null `source_expense_id` values existed in either database, so zero duplicates and nothing to clean up before adding the constraint.

**Migration**: `supabase/migrations/20260810060936_invoice_line_items_source_expense_uniqueness.sql` — a single additive, non-destructive partial unique index:
```sql
...
```
Same shape/pattern as the existing `invoices_one_deposit_per_job` (migration `20260802040000_deposit_invoice_uniqueness.sql`) — a partial unique index as the real concurrency authority, with the app-layer check kept as a friendly common-case pre-check, not removed.

**Applied to `premier-crm-e2e` only**, after a full name-by-name diff of local migration files against `list_migrations` confirmed this was the only pending gap (no unrelated pending migrations existed). Verified live, in a single self-cleaning transaction (rolled back regardless of outcome): two `NULL source_expense_id` rows inserted without error (nulls remain unrestricted, ordinary non-expense-backed line items are unaffected); a first real `source_expense_id` insert succeeded; a second insert with the *same* `source_expense_id` was rejected immediately with a real Postgres `23505 unique_violation`. Not applied to production.

`addExpenseChargeToInvoice`'s insert-error handling now translates `23505` into the same `"This expense has already been added to an invoice."` `VALIDATION_ERROR` the pre-check already returns (matching the existing `org-invites.ts`/`deposits.ts` convention for this exact translation pattern), so a losing concurrent request surfaces a controlled error, never a raw DB exception.

**Concurrency proof**: `tests/e2e/expense-invoice-integrity-bot.spec.ts`, test 6, fires two `addExpenseChargeToInvoice` calls at the same expense via `Promise.all` (not sequential awaits) — genuinely racing past the app-layer pre-check before either insert commits. Live result: exactly one call succeeds, exactly one fails with the controlled error, and exactly one `invoice_line_items` row exists afterward, referencing the winner's invoice. This is the real proof that the database — not request timing — is the final authority.

**"Add to invoice" success path — previously unreachable, now proven live**: independent verification's own report noted this exact click-through wasn't observable in the shared `premier-crm-e2e` org (no fixture job was simultaneously expense-eligible and paired with a draft, non-working invoice). `expense-invoice-integrity-bot.spec.ts` closes that gap with its own fully isolated, self-cleaning fixture (dedicated org/customer/property/job/user, torn down in `afterAll`) — test 7 logs in as that fixture's real owner account, clicks the real "Add to invoice" button, and confirms: the real `?message=Expense added to invoice.` server redirect, the "No line items yet." empty state disappearing, and a live DB query showing the exact expected `source_expense_id`/`invoice_id`/`name` on the resulting line item.
...
- Test data used for that manual verification pass (3 throwaway expenses named `E2E_VERIFY_FINANCE_EXPENSE`/`_2`, one of which had not accepted a void action and remained an orphaned draft) has since been **deleted directly** during this follow-up pass — identified unambiguously by its distinctive marker name, the exact timestamp of that verification session, zero invoice linkage, and zero downstream references. Confirmed zero residue via SQL: `0` rows matching that marker, `0` fixture organizations from this pass's own isolated test org (`E2E_EXPENSE_INVOICE_INTEGRITY_ORG` — created and torn down per `expense-invoice-integrity-bot.spec.ts` run), `0` non-null `source_expense_id` links remaining, `0` duplicate links.
...
### Follow-up pass: DB-level double-billing enforcement (this update)

**New spec**: `tests/e2e/expense-invoice-integrity-bot.spec.ts` — follows the self-contained service-role fixture pattern of `invoice-totals-recalc-bot.spec.ts` (a dedicated org/customer/property/job/user created in `beforeAll`, fully torn down in `afterAll`) rather than the shared `premier-crm-e2e` demonstration org, since the concurrency proof needs exact control over expense/invoice state. 7 tests, all passing live: eligibility-before-linkage, success + real `source_expense_id` persistence + real total recalculation + removal from the eligible list, sequential-duplicate rejection, **genuine concurrent-race rejection** (`Promise.all`, not sequential awaits — exactly one of two simultaneous `addExpenseChargeToInvoice` calls wins, confirmed via a live DB count), and the real browser "Add to invoice" click-through.

Two real bugs were found and fixed in this new spec during its own live verification (not product defects — both were test-authoring mistakes caught before the spec was considered done):
1. Test 5's expected error message was wrong — after a successful link, the expense's own `status` flips to `'invoiced'`, so a second attempt is actually rejected by the *eligibility* guard (fires earlier in `addExpenseChargeToInvoice`) rather than the duplicate-link guard. Fixed the assertion to match the real, correct guard-ordering behavior instead of asserting incorrect behavior.
2. Test 7's "Add to invoice" button locator used `.first()` unscoped — test 1's own fixture expense is deliberately never linked (to prove eligibility persists), so it remains permanently eligible on the same shared fixture job and rendered its own "Add to invoice" button ahead of test 7's expense in DOM order. The click silently linked the *wrong* expense to the invoice, and weak page-text assertions (checking for text that was already visible pre-click in the "Eligible expenses" card) let this pass invisibly the first time — caught only by the final DB assertion failing. Fixed by scoping the button locator to the specific `<li>` card containing the test's own expense description, and strengthened the assertions to check the real "Line items" section state and wait for the actual `?message=` URL redirect rather than relying on `networkidle` alone.

**Migration verification**: applied `20260810060936_invoice_line_items_source_expense_uniqueness.sql` to `premier-crm-e2e` only, confirmed via `list_migrations` name-by-name diff that it was the only pending gap; index existence, null-tolerance, normal-line-item compatibility, and duplicate rejection all verified live in one self-cleaning transaction (see the "Double-billing" section above for the exact proof).

**Regression**: `pnpm test` (391 passing, all green), `pnpm typecheck` (clean, 5 packages), `pnpm --filter web build` (clean), ESLint on changed files (clean). E2E regression run against `premier-crm-e2e`: `jobs-base44-shell-bot`, `calendar-base44-shell-bot`, `estimates-base44-shell-bot`, `quotes-base44-shell-bot`, `customers-base44-shell-bot`, `today-redesign-bot`, `today-kanban-board-semantics-bot` — 74 tests total, 71 passed on the first run, 3 failed with `net::ERR_ABORTED`/URL-timeout errors (classic worker-concurrency navigation flakes, not tied to any change in this pass); all 3 re-ran clean in isolation with `--workers=1`, confirming no real regression. Finance suite (`invoices-base44-shell-bot`, `expenses-base44-shell-bot`, `payments-flow-bot`, `expense-invoice-integrity-bot`): 26 passed, 3 honest skips, 0 failures.

**Residue cleanup**: the one previously-known orphaned draft expense (`E2E_VERIFY_FINANCE_EXPENSE`, id `1d9b97d5-...`) plus its two already-voided siblings were deleted directly via SQL, identified unambiguously by marker name and session timestamp. Zero residue confirmed: `0` matching expenses, `0` fixture organizations from this pass's own isolated test runs, `0` non-null `source_expense_id` links, `0` duplicate links.
...
2. ~~`addExpenseChargeToInvoice`'s double-billing guard is app-layer only~~ — **resolved.** See "Double-billing: closed at the database level" above — `invoice_line_items_source_expense_id_key` is now the real, DB-enforced authority, applied and verified live on `premier-crm-e2e`, with a real concurrency proof.
...
5. ~~The "Add to invoice" success path was not observed live~~ — **resolved.** `expense-invoice-integrity-bot.spec.ts` test 7 proves the real click-through end to end via a fully isolated, self-cleaning fixture.
6. ~~One harmless orphaned draft expense remains in `premier-crm-e2e`~~ — **resolved.** Deleted directly; zero residue confirmed via SQL (see Testing above).
...
7. docs: this report.
8. `37256f9` — fix: invoices/expenses table overflow + E2E selector/wait bugs found during independent verification.
9. db: `invoice_line_items_source_expense_id_key` unique partial index (migration `20260810060936`) + `23505` error translation in `addExpenseChargeToInvoice`, closing the double-billing gap at the database level.
10. tests: new `expense-invoice-integrity-bot.spec.ts` — eligibility, linkage, sequential-duplicate rejection, real concurrent-race rejection, and a live "Add to invoice" UI click-through, via a fully isolated self-cleaning fixture.
11. docs: this update — DB-authority section, resolved known-limitations, fresh live testing/regression results, residue cleanup.
