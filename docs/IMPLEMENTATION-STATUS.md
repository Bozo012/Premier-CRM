...
## Feature: team invites (2026-07-23) — superseded, see "Auth Reset" below
...
The original version of this feature accepted invites through a custom self-serve form: a public `/invite/[token]` page collected a password, called `supabase.auth.signUp()` directly, and `acceptOrgInvite` (the atomic RPC) finished the job. **This entire acceptance path — the `/invite/[token]` signup page, its `acceptInviteAction`, `AcceptTeamMemberInviteSchema`, and `sendTeamInviteEmail` — was removed in the "Auth Reset" pass below** in favor of Supabase's own admin invite flow. `org_invites` (the table), `accept_org_invite()` (the RPC), `createOrgInvite`/`listPendingInvites`/`revokeOrgInvite`/`resendOrgInvite`/`getInviteByToken`/`translateAcceptInviteError` (the query layer), and the `/team` UI (`InviteMemberForm`, pending-invites list, `RevokeInviteButton`) all remain — only how an invite gets *accepted* changed.
...
## Feature: Auth Reset — standard Supabase admin invitation architecture (2026-07-27)

Replaced the custom `signUp()` → confirm → continue acceptance chain above with Supabase's own server-side admin invitation flow, while keeping `org_invites`, `accept_org_invite()`, and `getActiveOrgContext()` unchanged. Also cleaned up production Auth accounts: obsolete/test accounts (`sommerskevin3@gmail.com`, `ksommers7@me.com`, `ksommers7@icloud.com`) and 4 test customer records were removed manually (via direct SQL, not a migration) after Kevin approved the dry-run classification; the 3 remaining accounts (`kevinsommers@ppmnky.com` owner, `e2e-admin-bot@example.com` admin, `delivered+e2e-employee-persistent@resend.dev` employee) were left untouched.

- **New-user flow**: `createInviteAction`/`resendInviteAction` (`apps/web/app/(app)/team/actions.ts`) look up the invited email via `findAuthUserByEmail()` (`apps/web/lib/auth-admin.ts`, a paginated `admin.listUsers()` scan — there's no direct "get by email" endpoint). If no confirmed account exists yet, they call `supabase.auth.admin.inviteUserByEmail(email, { redirectTo: '${APP_URL}/auth/accept-invite' })` — Supabase sends its own native invite email, never a custom one. The link lands on `/auth/accept-invite` (`apps/web/app/auth/accept-invite/page.tsx`), which establishes the session itself: Supabase's hosted verify endpoint redirects there with an implicit-flow session in the URL **hash fragment** (never a server-visible query param), and the page manually parses `window.location.hash` and calls `supabase.auth.setSession()` — required because `@supabase/ssr`'s `createBrowserClient()` (unlike plain `supabase-js`) does not auto-detect hash-fragment sessions; this was a real, previously-undiscovered bug found and fixed during this pass. Tokens are stripped from the address bar via `history.replaceState` immediately after, and are never logged or persisted outside Supabase's own storage. Once a session exists, the invitee sets a full name + password; `completeInviteAcceptanceAction()` (`apps/web/app/auth/accept-invite/actions.ts`) then resolves the invite strictly by the session's own server-verified email (`supabase.auth.getUser()`, never a client-supplied token/org/role) and calls `accept_org_invite()`. Idempotent: reopening after acceptance is detected (`status === 'accepted' && accepted_user_id === user.id`) and short-circuits to an "already set up" state rather than re-running acceptance.
- **Existing-user flow**: Supabase's `inviteUserByEmail()` is for brand-new addresses only. When `findAuthUserByEmail()` finds an already-**confirmed** account, the app sends its own single Resend email instead (`sendExistingUserJoinEmail()`, `apps/web/lib/email.ts` — the only custom email left in this flow) linking to `/invite/[token]/continue` (`apps/web/app/invite/[token]/continue/route.ts`, repurposed rather than rebuilt). That route requires the visitor to sign in with their existing password first (redirects unauthenticated visits to `/login?redirectTo=...`), then verifies: authenticated session, verified email matches the invite's email exactly, invite status is `pending`, invite is unexpired — before calling `accept_org_invite()`. An already-accepted invite for that same user redirects straight to `/today` without re-accepting; any other non-pending/expired/wrong-email state signs the visitor out and redirects to `/login` with a plain-text explanation (never HTML-injected). Never creates a second Auth account for an address that already has one.
- **Organization resolution unchanged**: `getActiveOrgContext()` (`packages/db/queries/org-context.ts`) remains the sole source of truth for "what org is this user acting in" — requires `status = 'active'`, and returns a `NOT_FOUND`/`CONFLICT` result rather than guessing when a user has zero or multiple active memberships. `handle_new_user()` (migration `20260727010000`) still only ever auto-creates a membership for a genuinely first-ever signup against an uninitialized org (as an already-active owner) — every other signup, invited or not, gets zero memberships until a real invite is accepted.
- **Security fix found during final review**: `apps/web/lib/auth-routing.ts`'s `normalizeRedirectPath()` (used by `/login?redirectTo=...`, which the existing-user join flow relies on) only checked for a single leading `/`, which let `//evil.com` and `/\evil.com` (both browser-resolved as a different origin) through as an open redirect. Fixed to reject a second `/` or `\` immediately after the leading slash; covered by `apps/web/lib/auth-routing.test.ts`.
- **Known, unaddressed concern**: `accept_org_invite()`'s `ON CONFLICT (org_id, user_id) DO UPDATE SET status = 'active'` does not update `role` — see the doc comment on `acceptOrgInvite()` in `packages/db/queries/org-invites.ts`. Unreachable today (no flow produces a stale differently-roled row), but must be revisited by the first role-management/member-removal feature.
- **Removed**: `apps/web/app/invite/actions.ts` (`acceptInviteAction`), `apps/web/app/invite/[token]/page.tsx` (the public signup page), `apps/web/app/auth/confirm/route.ts` (confirmed zero remaining references, including in the customer portal), `AcceptTeamMemberInviteSchema`, `sendTeamInviteEmail` (renamed/repurposed to `sendExistingUserJoinEmail`), the old `employee-invite-bot.spec.ts`, and its now-unused test helpers/env vars (`TEST_EMPLOYEE_*`).
- **Tests**: `tests/e2e/employee-onboarding-admin-invite-bot.spec.ts` replaces the deleted file — covers both onboarding paths end to end (real UI form submission, real `accept_org_invite()` calls, no service-role bypass of the acceptance step itself), plus duplicate/revoke/resend/expiry invite management and wrong-email/revoked/expired/reused-link coverage for the existing-user path. `apps/web/lib/auth-routing.test.ts` unit-tests `normalizeRedirectPath()` directly.
- Validation: `pnpm typecheck`/`pnpm lint`/`pnpm build` clean; `pnpm test` all green; full E2E suite re-run with only the 2 pre-existing, unrelated parallel-load flakes documented elsewhere in this file.
...
## Fix: /jobs/new and /quotes/new dead-end on unmatched customer (2026-07-24)

Reported bug, confirmed via manual click-through before touching anything: both pages only worked if the customer search returned an exact hit and got explicitly selected — there was no way to type a new customer's name/address directly, so "Create job"/"Create quote" stayed permanently disabled whenever the customer didn't already exist. This is also why the Invoices job picker showed "No jobs found" — nothing had ever been successfully created through this path.

- `CustomerPropertyWorkForm` (shared by `/estimates/new`, `/quotes/new`, `/jobs/new`) now has a "Search existing" / "Enter new customer" toggle. Manual entry combines name/contact/address in one step (a job/quote needs a property anyway) and reuses the exact soft-dedupe pattern from `/customers/new` (`checkCustomerEmailAction`) — a match shows "Use existing customer" / "Create new anyway" / "Cancel", no silent merge or duplicate. No new query-layer or schema code was needed — `createCustomerAction` and `createPropertyForCustomerAction` (already built for `/customers/new`) are reused as-is.
- An existing customer resolved via search now also gets an inline "+ Add a property" option instead of a dead-end when they have zero properties on file.
- **Real bug caught during the actual browser click-through, not just code review**: the manual-entry and add-property sections were each a `<form>` nested inside the page's outer work-details `<form>` — invalid HTML. Browsers resolve nested forms by falling back to a native top-level submission on the outer form, which wiped all React state and serialized every field into the URL as a GET request the instant "Continue" was clicked. This was the actual root cause of the dead end, not just the missing new-customer path. Fixed by converting both sections to plain `<div>`s with ref-based `FormData` construction and `onClick`/`onKeyDown` instead of `onSubmit`.
- **Verified end-to-end via a real authenticated browser session** — this app has no magic-link/PKCE callback route (only `signInWithPassword`/`signUp`), confirmed by testing an admin-generated magic link first and watching it bounce back to `/login`. Set a temporary password on `sommerskevin3@gmail.com` via the Supabase admin API (with explicit approval first), logged in through the real `/login` form, and drove the actual UI:
  - `/jobs/new`: brand-new customer typed inline → job created, linked correctly. Retyping the same email → dedupe match surfaced correctly, "Use existing customer" reused the same customer/property, no duplicate. Existing search-and-select path still works.
  - `/quotes/new`: same three paths, all confirmed (search-and-select, new-customer-inline, dedupe-match-reuse).
  - `/invoices` job picker: previously empty, now correctly lists the jobs created above; selected one and a real draft invoice was created.
  - All test customers/properties/jobs/quotes/invoices deleted afterward via SQL; verified back to zero. The temporary password remains set on the test account — not restored, since I never had the original; the user can reset it via "Forgot password" whenever convenient.
- Validation: `pnpm typecheck`/`test` (61/61)/`build`/`eslint` all clean.
