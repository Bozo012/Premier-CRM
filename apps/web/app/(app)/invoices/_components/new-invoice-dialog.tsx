...
import {
  ...
  searchJobsForInvoicePickerAction,
  type CreateInvoiceActionState,
  type JobPickerItem,
  type SearchJobsForInvoicePickerActionState,
} from '../actions';
...
export function NewInvoiceDialog() {
  ...
  const [searchState, searchFormAction, isSearching] = useActionState<
    SearchJobsForInvoicePickerActionState | null,
    FormData
  >(searchJobsForInvoicePickerAction, null);

  const [createState, createFormAction, isCreating] = useActionState<
    CreateInvoiceActionState | null,
    FormData
  >(createInvoiceFromJobAction, null);

  useEffect(() => {
    if (createState?.success) {
      router.push(`/invoices/${createState.data.invoiceId}`);
    }
  }, [createState, router]);

  useEffect(() => {
    if (createState && !createState.success) {
      toast.error(createState.error ?? 'Could not create invoice. Please try again.');
    }
  }, [createState]);
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-3">
        <form action={searchFormAction} className="flex gap-2">
          <Input name="q" placeholder="Search by title or job number…" className="flex-1" />
            ...
            {jobs.map((job) => (
              <li key={job.id} className="flex items-center gap-3 px-3 py-2.5">
                ...
                <form action={createFormAction} className="shrink-0">
                  ...
                  <Button type="submit" variant="outline" size="sm" disabled={isCreating}>
