...
// ---------------------------------------------------------------------------
// Job logs — staff-authored notes recorded in the shared activity_log table
// (the same authoritative timeline the job detail page already renders via
// getEntityTimeline). Deliberately staff-only: none of these event types are
// in activity-log.ts's CUSTOMER_VISIBLE_EVENT_COPY allow-list, so they never
// reach the customer portal timeline.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Job photos — reuses the private quarantine-then-finalize upload
// architecture built for site-visit/estimate attachments (packages/db's
// vault-items.ts + apps/web/lib/site-visit-attachments.ts), extended to
// accept `entityType: 'job'`. Same bucket, same signed-URL flow, same
// server-side content validation — no new storage/table contract.
// ---------------------------------------------------------------------------
...
export async function finalizeJobPhotoUploadAction(
  ...
  // org before finalizing it, same pattern as the site-visits worker's
  // finalizeSiteVisitPhotoUploadAction.
  const supabase = await getServerSupabase();
