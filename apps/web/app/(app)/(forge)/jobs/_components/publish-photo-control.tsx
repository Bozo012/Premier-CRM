...
import { publishJobPhotoAction, unpublishJobPhotoAction } from '../actions';
...
export function PublishPhotoControl({ vaultItemId, jobId, initialCustomerVisible, canPublish }: PublishPhotoControlProps) {
  ...
  const toggle = () => {
    startTransition(async () => {
      const result = customerVisible
        ? await unpublishJobPhotoAction(vaultItemId, jobId)
        : await publishJobPhotoAction(vaultItemId, jobId);
      if (!result.success) {
