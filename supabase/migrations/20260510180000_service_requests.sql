-- Premier CRM — Service requests and customer accounts
-- Migration: 20260510180000_service_requests
--
-- Introduces service_requests and customer_accounts tables.
-- The original 0012_ prefixed files defining these were never applied to the
-- live DB (both were skipped due to a naming collision). This file replaces
-- them as the canonical live migration with two deliberate corrections:
--
--   1. property_id is nullable — intake flow may not have a resolved property
--      at submission time; the createServiceRequest function always resolves
--      one, but the column constraint should not enforce what the application
--      already guarantees.
--
--   2. Removed the broken service_requests_website_unreviewed CHECK constraint
--      that permanently locked all website-source rows at status='new', making
--      them impossible to review, approve, or close.
--
-- Property address snapshot columns are also nullable to match the nullable
-- property_id (both are optional supplemental data; the FK is the authority).
...
CREATE TABLE public.service_requests (
  ...
  -- Property address snapshot at submission time (optional, mirrors property_id nullability).
...
CREATE INDEX service_requests_property_idx ON public.service_requests (property_id) WHERE property_id IS NOT NULL;
...
-- Org staff can read/write all requests in their org.
CREATE POLICY "internal_org_service_requests" ON public.service_requests
  FOR ALL USING (public.user_is_in_org(org_id));

-- Customer portal: logged-in customers can read their own requests.
...
-- Customer portal: customers can submit new requests via the portal source only.
...
-- Org staff can manage customer accounts for their org.
CREATE POLICY "internal_org_customer_accounts" ON public.customer_accounts
  FOR ALL USING (public.user_is_in_org(org_id));

-- Customers can read their own account record.
CREATE POLICY "customer_select_own_account" ON public.customer_accounts
  FOR SELECT USING (auth_user_id = auth.uid());

-- Customer portal: SELECT-only visibility into the canonical customer/property rows.
-- Scoped through customer_accounts — does not grant org membership.
...
-- ============================================================================
-- GRANTS — service_role (used by the public intake API edge, bypasses RLS)
-- ============================================================================
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.customer_properties TO service_role;
GRANT USAGE, SELECT ON SEQUENCE public.service_request_number_seq TO service_role;
GRANT EXECUTE ON FUNCTION public.next_service_request_number() TO service_role;
